...
export function rehypeAutolinkHeadings(options = {}) {
  ...
  const opts = {
    ...
    behavior: options.behaviour ?? options.behavior ?? "prepend",
    content: options.content ?? contentDefaults,
    group: options.group,
    test: options.test,
  ...
  const is = convertElement(opts.test)

  if (opts.behavior === "wrap") {
    opts.method = wrap
  } else if (opts.behavior === "before" || opts.behavior === "after") {
    opts.method = around
  } else {
    if (!opts.props) {
      opts.props = { ariaHidden: "true", tabIndex: -1 }
    }
  ...
  return function plugin(tree) {
    visit(tree, "element", (node, index, parent) => {
      if (headingRank(node) && hasProperty(node, "id") && is(node, index, parent)) {
        return opts.method(node, index, parent)
      }
    })
  }
  ...
  function inject(node) {
    node.children[opts.behavior === "prepend" ? "unshift" : "push"](
      create(node, extend(true, {}, opts.props), toChildren(opts.content, node))
    )
  ...
  function around(node, index, parent) {
    ...
    const link = create(node, extend(true, {}, opts.props), toChildren(opts.content, node))
    let nodes = opts.behavior === "before" ? [link, node] : [node, link]

    if (opts.group) {
      const grouping = toNode(opts.group, node)
  ...
  /** @type {import('unist-util-visit/complex-types').Visitor<Element>} */
  function wrap(node) {
    node.children = [create(node, extend(true, {}, opts.props), node.children)]
    return [SKIP]
  }
