...
Welcome to Part 22 of Up and Running with FastAPI. If you missed part 21, you can find it [here](/blog/serving-an-activity-feed-from-fastapi).
...
```jsx:title=<span>jsconfig.json</span>
{
  "compilerOptions": {
    "baseUrl": "src"
  },
...
A feed page that displays recent activity is standard for most modern applications. Even so, deciding what to display and how to display it can get tricky. Fortunately, `elastic-ui` is going to make a lot of the "how" decisions for us. Much of the code we're going to use can be pulled from the comment-list [docs](https://elastic.github.io/eui/#/display/comment-list), so feel free to play around with it first.
...
Funny enough, most of the content is defined outside our main component, making the actual `CleaningActivityFeed` component itself only 7 lines. That won't quite be the case once we incorporate real data from our backend, but it's interesting to note. Everything is baked directly into the `EuiCommentList` component as the `comments` prop. Each item in the `comments` array has some of, or all of, the following attributes:
...
<CodesandboxDisplay
  ...
  codesandboxProps={{
    src:
      "https://codesandbox.io/embed/phresh-frontend-part-9-designing-a-cleaning-feed-bkyfe?fontsize=14&hidenavigation=1&theme=dark",
...
<CodesandboxDisplay
  ...
  codesandboxProps={{
    src:
      "https://codesandbox.io/embed/phresh-frontend-part-9-designing-a-cleaning-feed-redux-slice-mp09c?fontsize=14&hidenavigation=1&theme=dark",
    style: "width:100%; height:500px; border:0; border-radius: 4px; overflow:hidden;",
...
<figure>
  <img src="./Part_22_Feed_simple.png" />
</figure>
...
<figure>
  <img src="./Part_22_Feed_Complete.png" />
</figure>
...
We've removed all dependencies from our `useEffect` hook, ensuring that it only executes upon first page load. Eslint will complain about the proper dependencies not being included, so we'll disable them to avoid having to see that annoying warning. We've also removed the
...
We've extracted the button rendering into a function for readability and rendered different buttons depending on whether or not there are more events to fetch. We enable the button and provide an event handler that executes the `handleLoadMore` function that we just added when it is clicked. The `handleLoadMore` function is responsible for getting the next page of data when needed.

It simply accesses the last item in our current feed, grabs the timestamp and sends it on to our action creator. Remember that we parse the timestamp with `moment`, so we don't need to worry about how that datetime is formatted.
...
<CodesandboxDisplay
  ...
  codesandboxProps={{
    src:
      "https://codesandbox.io/embed/phresh-frontend-part-9-using-the-custom-feed-hook-2gc08?fontsize=14&hidenavigation=1&theme=dark",
    style: "width:100%; height:500px; border:0; border-radius: 4px; overflow:hidden;",
