...
tags:
    ...
    - authentication
---
import {RegistrationFlowImage} from "src/components"
...
So how do we *keep* users logged in? That's where JWTs come in. [JSON Web Tokens](jwt.io) are an open, industry standard method for representing claims securely between two parties.
...
As the FastAPI [docs](https://fastapi.tiangolo.com/tutorial/security/) state, security is often a complex and "difficult" issue. Readers who are unfamiliar with standard authentication practices are *highly* encouraged to read the FastAPI docs on security, as they cover a large amount of jargon that will be useful in understanding the parts to come.
...
Readers who unfamiliar with token-based authentication are *highly* encouraged to read JWT.io's introductory article found [here](https://jwt.io/introduction/). For the initiated, the basics are as follows:
...
![open api docs](phresh-part-7.png)
...
+ https://github.com/Jastor11/phresh-tutorial/tree/tutorial-part-7-user-auth-in-fastapi-with-jwt
