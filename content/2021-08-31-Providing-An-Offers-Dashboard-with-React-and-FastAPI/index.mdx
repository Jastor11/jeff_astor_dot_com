...
Welcome to Part 25 of Up and Running with FastAPI. If you missed part 24, you can find it [here](/blog/refactoring-our-react-ui-into-composable-hooks-ii").
...
<img src="https://i.stack.imgur.com/kwPTe.jpg" />
...
class TestGetCleaning:
    async def test_get_cleaning_by_id(
        ...
        cleaning = CleaningPublic(**res.json()).dict(exclude={"owner", "offers", "total_offers"}) # highlight-line
...
class CleaningsRepository(BaseRepository):
    ...
    async def populate_cleaning(
        ...
        *,
        ...
        """
        Cleaning models are populated with the owner and total number of offers made for it.

        If the user is the owner of the cleaning, offers are included by default.
        Otherwise, only the offers made by the requesting user are included.
        """
        ...
        return CleaningPublic(
            **cleaning.dict(exclude={"owner"}),
            ...
            offers=offers
            if populate_offers
            else [o for o in offers if requesting_user and o.user_id == requesting_user.id],
...
class CleaningsRepository(BaseRepository):
    ...
    async def populate_cleaning(
        ...
        *,
        ...
        If the user is the owner of the cleaning, offers are included by default.
        Otherwise, only the offer made by the requesting user is included
        """
        offers = await self.offers_repo.list_offers_for_cleaning(
            cleaning=cleaning,
            populate=populate_offers,
        ...
        return CleaningPublic(
            **cleaning.dict(exclude={"owner"}),
            owner=await self.users_repo.get_user_by_id(user_id=cleaning.owner),
            total_offers=len(offers),
            # full offers if `populate_offers` is specified, otherwise only the offer from the authed user
            offers=offers if populate_offers else [
                offer for offer in [await self.offers_repo.get_offer_for_cleaning_from_user(
                    cleaning=cleaning,
...
<!-- populating the offer returned by the `create_offer_for_cleaning` method. -->
...
async def create_offer(
    ...
    return OfferPublic(**offer.dict(), user=current_user)
    # highlight-end
...
class TestGetCleaning:
    async def test_get_cleaning_by_id(
        ...
        assert res.status_code == status.HTTP_200_OK
        # highlight-start
        cleaning = CleaningPublic(**res.json()).dict(exclude={"owner", "offers", "total_offers"})
        assert cleaning == test_cleaning.dict(exclude={"owner", "offers", "total_offers"})
        # highlight-end
